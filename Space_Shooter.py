...
YELLOOW_LASER = pygame.image.load(os.path.join("Assets", "pixel_laser_yellow.png"))
...
def main():
    ...
    lost_font = pygame.font.SysFont ("courier-new", 60)
    ...
    while run:
        ...
        if lost:
            if lost_count > FPS * 3:  # Show lost screen for 3 seconds
                run = False
            else:
                 continue
        ...
        for enemy in enemies[:]:
            ...
            if random.randrange(0, 2*120) == 1:  # Randomly shoot
                enemy.shoot()
...
>>>>>>> ae0dd17 (Initial commit with Assets folder and game files)
main_menu()